...
import lisp.mal_readline as mal_readline
import lisp.mal_types as types
import lisp.reader as reader, lisp.printer as printer
from lisp.env import Env
import lisp.core as core
...
def make_lisp_vm():
    ...
    for k, v in core.ns.items(): env.set(types._symbol(k), v)
    env.set(types._symbol('eval'), lambda ast: EVAL(ast, env))
    env.set(types._symbol('*ARGV*'), types._list(*sys.argv[2:]))
    ...
    REP("(def! *where-am-i* \"inside a blockchain\")")
