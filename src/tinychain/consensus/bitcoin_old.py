...
# 
# In terms of data we must store to verify the longest chain:
# 1. The genesis block.
# 2. The block DAG.
# 3. For each item in the block DAG, the following metadata:
#  - The block's height in the DAG.
#  - The accumulated work in this path.
#  - The block's timestamp.
#  - The current difficulty.
#  - The parent block.
#  - The child block - if this is null, it means it is the current tip of the chain.
# 
...
if __name__ == '__main__':
    ...
    for block in chain:
        # dump to test-data/bitcoin_chain1.yaml
        open("test-data/bitcoin_chain1.yaml", "a").write(encode_block_yaml(block))
    ...
    # test A:
    # 1. mine 2 chains
    # 2. compute acc difficulty of each
    # 3. get_tip should retrieve chain with highest acc difficulty
    # 4. get_tip should download missing blocks
